try:
    ...
    # 向后兼容：使引用 PyQt5 风格枚举（如 QtCore.Qt.*）的代码能在 PyQt6 下工作
    try:
        from PyQt6.QtCore import Qt as _Qt
        QtCore.Qt = _Qt
        # 将 PyQt5 常用枚举名尽量映射到 PyQt6 的等价项
        try:
            QtCore.Qt.WaitCursor = _Qt.CursorShape.WaitCursor
        except Exception:
            pass
        # 上下文菜单策略
        try:
            QtCore.Qt.CustomContextMenu = _Qt.ContextMenuPolicy.CustomContextMenu
        except Exception:
            pass
        # 窗口标志
        try:
            QtCore.Qt.WindowContextHelpButtonHint = _Qt.WindowType.WindowContextHelpButtonHint
        except Exception:
            pass
        # QMessageBox 标准按钮/图标兼容性
        try:
            from PyQt6.QtWidgets import QMessageBox as _QMessageBox
            if not hasattr(QtWidgets.QMessageBox, 'Yes'):
                QtWidgets.QMessageBox.Yes = _QMessageBox.StandardButton.Yes
            if not hasattr(QtWidgets.QMessageBox, 'No'):
                QtWidgets.QMessageBox.No = _QMessageBox.StandardButton.No
            if not hasattr(QtWidgets.QMessageBox, 'information'):
                # 保持方法名兼容（无需修改）
                pass
        except Exception:
            pass
...
class Pan123:
    ...
    def get_dir_by_id(self, file_id, save=True, all=False, limit=100):
        ...
        while (lenth_now < total or total == -1) and (times < get_pages or all):
            ...
            if times % 5 == 0:
                ...
                logger.warning("为防止对服务器造成影响，暂停3秒")
                logger.warning("请耐心等待！")
                time.sleep(3)
    ...
    def show(self):
        ...
        if not self.all_file:
            logger.info("剩余 %s 个文件未获取", self.total - len(self.list))
            logger.info("输入more继续获取")
        logger.info("--------------------")
    ...
    def link_by_fileDetail(self, file_detail, showlink=True):
        ...
        if res_code_download != 0:
            logger.error("code = 3 Error: %s", res_code_download)
    ...
    def download_from_url(self, url, file_name, download_path="download"):
        if os.path.exists(download_path + "/" + file_name):
            if self.download_mode == 4:
                logger.info("文件 %s 已跳过", file_name)
                return
            logger.info("文件 %s 已存在，是否要覆盖？", file_name)
        ...
        logger.info("%s    %s", file_name, size_print_download)
        time1 = time.time()
        time_temp = time1
        data_count_temp = 0
    ...
    def delete_file(self, file, by_num=True, operation=True):
        ...
        logger.debug("delete response: %s", dele_json)
    ...
    def share(self):
        ...
        while str(add) == "1":
            ...
            if num_test2:
                ...
                if 0 < share_num < len(self.list) + 1:
                    ...
                    logger.info("当前分享文件列表: %s", share_name_list)
        ...
        if str(add) == "0":
            ...
            if share_res_json.get("code", -1) != 0:
                logger.error(share_res_json.get("message", ""))
                logger.error("分享失败")
                return
    ...
    def up_load(self, file_path):
        ...
        logger.info("文件名: %s", file_name)
        if not os.path.exists(file_path):
            logger.warning("文件不存在，请检查路径是否正确")
            return
        if os.path.isdir(file_path):
            logger.warning("暂不支持文件夹上传")
            return
        ...
        if res_code_up == 0:
            ...
            logger.error("upload request failed: %s", up_res_json)
            logger.error("上传请求失败")
            return
        ...
        logger.info("上传文件的fileId: %s", up_file_id)
        ...
        if res_code_up == 0:
            ...
            logger.error("start_data: %s", start_data)
            logger.error("start_res_json: %s", start_res_json)
            logger.error("获取传输列表失败")
            return
    ...
    def mkdir(self, dirname, remakedir=False):
        ...
        try:
            ...
            logger.error("创建失败")
            logger.error("res_mk.text: %s", res_mk.text)
            return
        ...
        logger.error("mkdir failed: %s", res_json)
        logger.error("创建失败")
        return
